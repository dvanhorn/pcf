...
(define (make-pcf-tests -->v typable?)
  ...
  (test-->> -->v '(pos? 0) '1)
  (test-->> -->v '(pos? 1) '0)
  (test-->> -->v '(quotient 1 0) '(err #f nat "Divide by zero"))
  (test-->> -->v '(add1 (quotient 1 0)) '(err #f nat "Divide by zero"))
  ...
  (test-->> -->v '((err #f (nat -> nat) "l") (err #f nat "r"))
                 '(err #f (nat -> nat) "l"))
  (test-->> -->v '((λ ([x : nat]) x) (err #f nat "r"))
                 '(err #f nat "r"))
  (test-->> -->v '(((λ ([f : (nat -> nat)])
                      (λ ([x : nat])
                        (f (f x))))
